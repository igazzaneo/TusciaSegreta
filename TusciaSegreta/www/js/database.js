...
function setSitoInfo(sito) {
  ...
  var caratteristiche = "<div class=\"divTable\"><div class=\"divTableRow\"><div class=\"divTableHead1\"><i class='fas "+ sito[8][2] +"'></i>"+ sito[8][0] +"</div><div class=\"divTableHead2\"><i class='fas "+ sito[9][2] +"'></i>" + sito[9][0] + " km</div><div class=\"divTableHead3\"><i class='fas "+ sito[10][2] +"'></i>" + sito[10][0] + " min</div></div></div>";
...
/* funzione per prelevare i nodi da visualizzare dinamicamente sul percorso con multimedia o semplici info di percorso */
...
function getNodiPercorsoSito(id, database, map, callback)
{
  if(database != null) {
    database.transaction(function(transaction) {
      ...
      transaction.executeSql('SELECT nodo.id, nodo.latitudine, nodo.longitudine, nodo.percorso_id, nodo.descrizione, nodo.nome,	multimedia.descrizione as multimedia_descr, multimedia.tipo_multimedia_id, multimedia.oggetto from nodo inner join percorso on percorso.id=nodo.percorso_id left join nodo_ha_multimedia on nodo.id=nodo_ha_multimedia.nodo_id left join multimedia on multimedia.id=nodo_ha_multimedia.multimedia_id where percorso.sito_id=? and (multimedia.tipo_multimedia_id is null or multimedia.tipo_multimedia_id<>9)', [id], function(transaction, resultSet) {
        var elenco = new Array();
        ...
        for(var x = 0; x < resultSet.rows.length; x++) {
          ...
          riga[6] = resultSet.rows.item(x).multimedia_descr;
          riga[7] = resultSet.rows.item(x).tipo_multimedia_id;
          riga[8] = resultSet.rows.item(x).oggetto;
...
function getNodiStaticiPercorsoSito(id, database, map, callback)
{
  if(database != null) {
    database.transaction(function(transaction) {
      ...
      //transaction.executeSql('SELECT nodo.id, nodo.latitudine, nodo.longitudine, nodo.percorso_id, nodo.descrizione, nodo.nome from nodo inner join percorso on percorso.id=nodo.percorso_id where percorso.sito_id=?', [id], function(transaction, resultSet) {
      transaction.executeSql('SELECT nodo.id, nodo.latitudine, nodo.longitudine, nodo.percorso_id, nodo.descrizione, nodo.nome,	multimedia.descrizione, multimedia.tipo_multimedia_id from nodo inner join percorso on percorso.id=nodo.percorso_id left join nodo_ha_multimedia on nodo.id=nodo_ha_multimedia.nodo_id left join multimedia on multimedia.id=nodo_ha_multimedia.multimedia_id where percorso.sito_id=? and multimedia.tipo_multimedia_id=9', [id], function(transaction, resultSet) {
        var elenco = new Array();
